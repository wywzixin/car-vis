...
// // 用于镜头光晕的纹理对象
let texture = texLoader.load('model/lensflare.jpg');

// 创建两个镜头光晕Lensflare对象，分别用来模拟车前面左右两个车灯的发光效果，具体位置查看model.js文件
let lensflareElement = new LensflareElement(texture, 600, 0, new THREE.Color(0xffffff)); //600:设置光晕像素大小
let lensflare1 = new Lensflare();

lensflare1.addElement(lensflareElement); // Lensflare可以根据需要包含多个LensflareElement
let lensflare2 = new Lensflare();
