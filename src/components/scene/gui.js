...
//关联空间数据创建交互界面
let gui = new dat.GUI({ width: 330 }); //创建GUI对象
let folder1 = gui.addFolder('高光金属——车金属边条、轮毂');
folder1.open();
//设置交互界面位置
...
let folder2 = gui.addFolder('镜子材质');
folder2.open();
folder2
    .add(guiControls, 'jingmetalness', 0.0, 1.0)
    .name('metalness')
    .onChange(function (value) {
        setMaterial('后视镜', 'metalness', value);
    });
folder2
    .add(guiControls, 'jingroughness', 0.0, 1.0)
    .name('roughness')
    .step(0.01)
    .onChange(function (value) {
...
folder2
    .add(guiControls, 'jingenvMapIntensity', 0.0, 5)
    .name('envMapIntensity')
    .onChange(function (value) {
        setMaterial('后视镜', 'envMapIntensity', value);
    });

let folder3 = gui.addFolder('车外壳油漆');
folder3.open();
folder3
    .add(guiControls, 'kemetalness', 0.0, 1.0)
    .name('metalness')
    .onChange(function (value) {
        setMaterial('外壳', 'metalness', value);
    });
folder3
    .add(guiControls, 'keroughness', 0.0, 1.0)
    .name('roughness')
    .step(0.01)
    .onChange(function (value) {
...
folder3
    .add(guiControls, 'keenvMapIntensity', 0.0, 5)
    .name('envMapIntensity')
    .onChange(function (value) {
