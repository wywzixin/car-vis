...
// 创建一个非位置音频对象  用来控制播放
let openSound = new THREE.Audio(listener); //开门音频对象
let closeSound = new THREE.Audio(listener); //关门音频对象
// 创建一个音频加载器对象
...
function open(carModel) {
    let tagNameArr = ['右前光标', '右后光标', '左前光标', '左后光标', '后备箱光标'];
    let doorNameArr = ['右前门', '右后门', '左前门', '左后门', '后备箱'];
    let chooseArr = [];
    tagNameArr.forEach(function (tagName, i) {
        let tagObj = carModel.getObjectByName(tagName).children[0];
    ...
    function choose(event) {
        ...
        let intersects = raycaster.intersectObjects(chooseArr);
        console.log('射线器返回的对象', intersects);
        // intersects.length大于0说明，说明选中了模型
        if (intersects.length > 0) {
            //选中的车门
            let chooseDoor = intersects[0].object.door;
...
function openClose(axis, angle1, angle2, door, openClose) {
    ...
    let tween = new TWEEN.Tween(state); //创建一段tween动画
    tween.to(
        {
